...
(function( $, window, document, undefined ) {
	...
	var SiteSlide = {
		init: function (options, elem) {
			...
			self.options = $.extend( {}, $.fn.SiteSlide.defaults, options );
            self.$elem = $(elem);
            ...
            var $tr = $('<tr>').append(
                ...
                , $('<td>', {class: 'td-data'}).append(
                    ...
                    $('<div>', {class: 'media'}).append(
                          '<div class="pic-wrap mr-3" style="width: 350px"><div class="pic"></div></div>'
                        , $('<div>', {class: 'media-body'}).append(
                            ''

                            , '<label class="control-label">กำหนดรายละเอียดรูปภาพ</label>'
                            , $('<div>', {class: 'mb-1'}).html( $('<input>', {class: 'form-control form-title', 'data-name': 'images[index][title]', maxLength: 200, placeholder: "เพิ่มหัวข้อ"}) )

                            // , '<label class="control-label"></label>'
                            , $('<div>', {class: 'mb-1'}).html( $('<textarea>', {class: 'form-control form-caption', 'data-name': 'images[index][caption]', 'data-plugin': 'autosize', rows: 2, maxLength: 200, placeholder: "คำบรรยายรูป (ไม่เกิน 200 อักษร)"}) )

                            // , '<label class="control-label">URL</label>'
                            , $('<div>', {class: 'mb-1'}).html( $('<input>', {class: 'form-control form-permalink', 'data-name': 'images[index][permalink]', maxLength: 200, placeholder: "เพิ่ม URL"}) )
	...
	$.fn.SiteSlide = function( options ) {
		return this.each(function() {
			var $this = Object.create( SiteSlide );
			$this.init( options, this );
			$.data( this, 'SiteSlide', $this );
		});
	};
	$.fn.SiteSlide.defaults = {
	}

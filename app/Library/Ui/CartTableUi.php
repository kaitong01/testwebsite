...
class CartTableUi extends Ui
{
    ...
    public function _action($data)
    {
			...
			if($data['status']!=2&&$data['status']!=0){
					$val .= '<a href="'.asset('cart/status/'.$data['wh_series_id']).'/published"  class="btn btn-sm btn-primary">เผยแพร่</a>';
			}
			if($data['status']!=2){
				$val .= ' <a href="'.asset('cart/status/'.$data['wh_series_id']).'/cancel"  class="btn btn-sm btn-danger">ยกเลิก</a>';
			}

...
class TourRouteDatatable extends Ui
{
    ...
    public function groupName($data)
    {
        ...
        $picture = !empty($data->image_url)
            ? '<img class="img-h" src="'. $data->image_url. '" alt="" />'
            : '';

        $countryList = '';
        if( !empty($data->countries) ){
            foreach ($data->countries as $country) {
                $countryList .= '<li class="nav-item"><span>'.$country->name.'</span></li>';
            }
        }
    	...
    	return '<div class="media">'.
            ...
            '<div class="media-body">'.
                ...
                ( !empty($countryList)? '<ul class="nav">'.$countryList.'</ul>':'' ).
			'</div>'.
		'</div>';
    }

...
class TourCountryDatatable extends Ui
{
    ...
    {
        $original_name = !empty($data->original_name_th)? $data->original_name_th: $data->original_name;

        $name = !empty($data->name)? $data->name: $original_name;
        $description = !empty($data->description)? $data->description: $data->original_description;

        $picture = !empty($data->image_url)
    	...
    	return '<div class="media">'.
            ...
            '<div class="media-body">'.
                ...
                '<a href="'.asset($this->path.'/'.$data->original_id).'/edit" data-plugin="lightbox">'.
    ...
    public function toggleSwitch($data, $name='status')
    {
        ...
        $id = isset($data->{$this->primatyKey})? $data->{$this->primatyKey}:null;
    ...
    public function action($data)
    {
        ...
        $btn .= '<a href="'.asset( $this->path. '/'.$data->original_id).'/edit" data-plugin="lightbox" class="btn btn-sm btn-primary ml-1" title="แก้ไข"><i class="fa fa-pencil"></i></a>';

...
class DatacenterSeriesDatatable extends Ui
{
    ...
    public function action($data)
    {
        ...
        if( !empty( $data->cart ) ){
            ...
            if( $data->cart->status==2 ){
                $btn .= '<button type="button" class="btn btn-success"><i class="fa fa-check"></i><span class="ml-1">วางจำหน่ายแล้ว</span></button>';
            }
            else{
                $btn .= '<button type="button" class="btn btn-warning"><i class="fa fa-check"></i><span class="ml-1">อยู่ในตะกร้าแล้ว</span></button>';
            }
            ...
            $btn .= '<button type="button" class="btn btn-primary" data-plugin="SwitchCart" data-options="'. htmlentities(json_encode([

                'wh_series_id' => $data->id,
                'status' => 1,

            ])) .'"><i class="fa fa-plus"></i><span class="ml-1">เพิ่มลงตะกร้า</span></button>';
        }

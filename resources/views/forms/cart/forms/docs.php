...
foreach ($filesOps as $i => $item){
    ...
    $file_id = '';
    if( !empty( $series->files ) ){

        $_files = json_decode($series->files, 1);
        // dd($_files);
    ...
    $btnEdit = '';
    if( !empty($item['src']) ){
        $btnEdit = '<a class="btn btn btn-sm btn-light ml-1" href="'.$item['src'].'" target="_blank" title="ดาวน์โหลดไฟล์" role="download"><i class="fa fa-download"></i> ดาวน์โหลด</a>';

        $btnEdit = '<button data-action="undo" type="button" title="ใช้รูปเดิม" tabindex="-1" class="btn ml-1 btn btn-sm btn-light d-none"><i class="fa fa-refresh"></i> ใช้ไฟล์เดิม</button>';
    }

    $docs .= '<tr data-plugin="seriesFilesForm" data-options="'.htmlentities(json_encode($item)).'">
...
$doc = '<div id="files_fieldset" class="control-group">
    <div class="controls">
